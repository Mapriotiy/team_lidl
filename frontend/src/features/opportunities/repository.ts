...
export async function listOpportunities(
  ...
  const result = await listOpportunityRecords({ profile_id: filters.service, status: filters.status === 'all' ? undefined : filters.status, eligibility: filters.eligibility === 'all' ? undefined : filters.eligibility, search: filters.query, min_score: filters.minScore, sort: filters.sort ?? 'score_desc', page: filters.page ?? 1, page_size: 25 })
