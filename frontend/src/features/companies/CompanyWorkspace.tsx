...
import { translateEvidence } from '../../api/translations'
...
function EvidenceCard({ evidence }: { evidence: Evidence }) {
  ...
  const [translation, setTranslation] = useState(evidence.translations?.find((item) => item.targetLanguage === 'ro')?.text ?? '')
  const [translationError, setTranslationError] = useState('')
  const [translating, setTranslating] = useState(false)
  const changeLanguage = async (next: 'en' | 'ro') => {
    setLanguage(next); setTranslationError('')
    if (next === 'en' || translation) return
    setTranslating(true)
    try { setTranslation((await translateEvidence(evidence.id, 'ro')).translated_excerpt) } catch (error) { setTranslationError(errorMessage(error)) } finally { setTranslating(false) }
  }
  return (
    <article className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
      ...
      <div className="mt-3 flex items-center gap-2"><span className="text-xs text-slate-500">Display:</span>{(['en', 'ro'] as const).map((item) => <button aria-pressed={language === item} className={`rounded px-2 py-1 text-xs font-semibold ${language === item ? 'bg-cyan-300 text-slate-950' : 'bg-slate-800 text-slate-300'}`} key={item} onClick={() => void changeLanguage(item)}>{item.toUpperCase()}</button>)}</div>
      {language === 'ro' && <div className="mt-3 rounded-lg bg-slate-900 p-3"><p className="text-[11px] font-semibold uppercase tracking-wide text-violet-300">Romanian translation</p><p className="mt-1 text-sm text-slate-300">{translating ? 'Translating…' : translation || 'Translation unavailable'}</p>{translationError && <p className="mt-2 text-xs text-amber-300">{translationError}. Original evidence remains available above.</p>}</div>}
...
export function CompanyWorkspace({ companyId = 'company-lufthansa', opportunityId, opportunityStatus, opportunityNote }: { companyId?: string; opportunityId?: string; opportunityStatus?: 'new' | 'shortlisted' | 'dismissed'; opportunityNote?: string | null }) {
  ...
  return (
    <div>
      ...
      {(company.warnings?.length || company.exclusions?.length) && <div className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/5 px-4 py-3 text-sm text-amber-100/80">{company.exclusions?.map((item) => <p key={item}>Excluded: {item}</p>)}{company.warnings?.map((item) => <p key={item}>Warning: {item}</p>)}</div>}
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_310px]">
        ...
        <aside className="space-y-5">
          ...
          <OpportunityActions initialNote={opportunityNote ?? ''} initialStatus={opportunityStatus} opportunityId={opportunityId} />
