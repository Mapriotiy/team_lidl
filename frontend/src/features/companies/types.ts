...
export interface ResearchSource { id: string; title: string; url: string; type: string; retrievedAt: string; publicationDate: string | null }
...
export interface CompanyDetail { id: string; name: string; domain: string; aliases: string[]; profileName: string; score: number; coverage: number; eligibility: string; facts: CompanyFact[]; sources?: ResearchSource[]; evidence: Evidence[]; assessments: Assessment[]; contributions: Contribution[]; researchRuns: ResearchRun[]; warnings?: string[]; exclusions?: string[]; penaltyPoints?: number }
