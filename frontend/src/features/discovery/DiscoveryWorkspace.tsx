...
type DiscoverySortKey = 'company' | 'country' | 'industry' | 'employees' | 'match'
...
export function DiscoveryWorkspace({ onActivity, onProfile, onResearchQueued }: { onActivity?: () => void; onProfile?: () => void; onResearchQueued?: () => void }) {
  ...
  const [sort, setSort] = useState<{ key: DiscoverySortKey; direction: SortDirection }>({ key: 'match', direction: 'desc' })
  ...
  const filtered = candidates.filter((item) => `${item.name} ${item.domain}`.toLowerCase().includes(query.toLowerCase()) && (!country || item.country_code === country) && (!industry || (industry === 'unknown' ? !item.industry : item.industry === industry)) && (!size || (size === 'unknown' ? item.employee_count === null : size === 'small' ? item.employee_count !== null && item.employee_count < 250 : size === 'medium' ? item.employee_count !== null && item.employee_count >= 250 && item.employee_count < 1000 : item.employee_count !== null && item.employee_count >= 1000)))
    .sort((a, b) => {
      ...
      const comparison = sort.key === 'company' ? a.name.localeCompare(b.name)
        ...
        : sort.key === 'employees' ? (a.employee_count ?? 0) - (b.employee_count ?? 0)
        : profile ? match(a, profile).score - match(b, profile).score : 0
