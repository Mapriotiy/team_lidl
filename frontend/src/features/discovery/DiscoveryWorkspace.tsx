...
import { describeCriteria, match, profileSearch } from './matching'
...
type DiscoverySortKey = 'company' | 'country' | 'industry' | 'employees'
...
export function DiscoveryWorkspace({ onActivity, onProfile, onResearchQueued }: { onActivity?: () => void; onProfile?: () => void; onResearchQueued?: () => void }) {
  ...
  const [sort, setSort] = useState<{ key: DiscoverySortKey; direction: SortDirection }>({ key: 'company', direction: 'asc' })
  ...
  const toggleSort = (key: DiscoverySortKey) => setSort((current) => ({
    key,
    direction: current.key === key ? (current.direction === 'asc' ? 'desc' : 'asc') : key === 'company' || key === 'country' || key === 'industry' ? 'asc' : 'desc',
  }))
  ...
  const filtered = candidates.filter((item) => `${item.name} ${item.domain}`.toLowerCase().includes(query.toLowerCase()) && (!country || item.country_code === country) && (!industry || (industry === 'unknown' ? !item.industry : item.industry === industry)) && (!size || (size === 'unknown' ? item.employee_count === null : size === 'small' ? item.employee_count !== null && item.employee_count < 250 : size === 'medium' ? item.employee_count !== null && item.employee_count >= 250 && item.employee_count < 1000 : item.employee_count !== null && item.employee_count >= 1000)))
    .sort((a, b) => {
      const missing = sort.key === 'employees' && (a.employee_count === null || b.employee_count === null)
      if (missing && a.employee_count !== b.employee_count) return a.employee_count === null ? 1 : -1
      const comparison = sort.key === 'company' ? a.name.localeCompare(b.name)
        : sort.key === 'country' ? a.country_name.localeCompare(b.country_name)
        : sort.key === 'industry' ? (a.industry ?? '\uffff').localeCompare(b.industry ?? '\uffff')
  ...
  return <section className="pb-28 text-[#20242A]" aria-label="Discover companies">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-2xl font-semibold">Discover companies</h2><p className="mt-2 text-sm text-[#68645F]">Companies are discovered automatically from the active Service Profile criteria.</p></div><div className="flex flex-wrap items-end gap-2"><button className={button} onClick={() => setAdding(!adding)}>Add company by website</button><button className={button} disabled={loading || busy || !profile} onClick={refresh}>Refresh results</button></div></div>
    {profile && <div className="mt-5 rounded-xl border border-[#DED9D1] bg-white p-4 text-sm"><p><span className="font-semibold">Saved criteria: </span>{describeCriteria(profile)}</p><p className="mt-1 text-xs text-[#73706A]">Ordered by available company facts. Buying signals and disqualifiers are assessed during research.</p>{notes.map((note) => <p className="mt-1 text-xs text-[#73706A]" key={note}>{note}</p>)}{onProfile && <button className="mt-2 text-sm font-semibold text-[#B64B16]" onClick={onProfile}>Edit Service Profile</button>}</div>}
    ...
    {!loading && snapshot && <><p className="mt-5 text-xs text-[#73706A]">Showing {shown.length} of {filtered.length} companies · Click a column heading to sort · Updated {new Date(snapshot.run.created_at).toLocaleString()}</p><div className="mt-3 overflow-x-auto rounded-xl border border-[#CFC7BC] bg-white shadow-[0_6px_20px_rgba(58,45,31,0.06)]"><table className="w-full text-left text-sm"><thead className="border-b-2 border-[#C8BFB3] bg-[#E8E2DA] text-xs text-[#4F4943]"><tr><th className="p-4"><span className="sr-only">Select</span></th>{([['company', 'Company'], ['country', 'Country'], ['industry', 'Industry'], ['employees', 'Employee count']] as Array<[DiscoverySortKey, string]>).map(([key, label]) => <th className="px-4 py-3 font-bold" key={key}><button type="button" aria-label={`Sort by ${label}${sort.key === key ? `, ${sort.direction === 'asc' ? 'ascending' : 'descending'}` : ''}`} className="flex items-center gap-1 whitespace-nowrap hover:text-[#A64212]" onClick={() => toggleSort(key)}>{label}<span aria-hidden="true">{sort.key === key ? sort.direction === 'asc' ? '↑' : '↓' : '↕'}</span></button></th>)}</tr></thead><tbody>{shown.map((item) => <tr className="cursor-pointer border-b border-[#DDD6CD] transition even:bg-[#FBF9F6] last:border-0 hover:bg-[#FFF0E5]" key={item.domain} onClick={() => setDetail(item)}><td className="p-4" onClick={(event) => event.stopPropagation()}><input type="checkbox" aria-label={`Select ${item.name}`} checked={selected.includes(item.domain)} disabled={busy || (!selected.includes(item.domain) && selected.length >= 10)} onChange={() => toggle(item.domain)} className="size-4 accent-[#E86722]" /></td><td className="min-w-48 px-4 py-4"><button className="text-left font-semibold text-[#292521] hover:underline" onClick={(event) => { event.stopPropagation(); setDetail(item) }}>{item.name}</button><p className="mt-1 text-xs text-[#68615B]">{item.domain}</p></td><td className="px-4 py-4">{item.country_name}</td><td className="px-4 py-4">{item.industry || 'Unknown'}</td><td className="min-w-40 px-4 py-4 text-[#625D57]">{employeeLabel(item)}</td></tr>)}</tbody></table>{!shown.length && <div className="p-12 text-center"><h3 className="font-semibold">{candidates.length ? 'No companies match these filters' : 'No companies found'}</h3><p className="mt-2 text-sm text-[#73706A]">{candidates.length ? 'Clear or adjust the filters to see more results.' : 'Try refreshing, review your Service Profile, or add a company by website.'}</p></div>}</div>{shown.length < filtered.length ? <button className={`${button} mt-4`} onClick={() => setSnapshot((current) => current ? { ...current, visible: current.visible + 25 } : current)}>Load more</button> : candidates.length > 0 && <p className="mt-4 text-xs text-[#73706A]">All available results shown{candidates.length >= 100 ? ' (up to 100 per search)' : ''}.</p>}</>}
