...
function ResearchCompanyList({ companies, onOpen }: { companies: CompanyDetail[]; onOpen: (company: CompanyDetail) => void }) {
  return <section className="overflow-hidden rounded-2xl border border-[#DCD7CF] bg-white shadow-[0_8px_28px_rgba(58,45,31,0.06)]">
    <div className="grid grid-cols-[minmax(0,1fr)_160px_170px_120px] gap-5 border-b border-[#E5E0D8] bg-[#FCFBF8] px-6 py-3 text-[11px] font-bold uppercase tracking-wider text-[#817A72]"><span>Company</span><span>Status</span><span>Last updated</span><span className="text-right">Research</span></div>
    <div className="divide-y divide-[#E9E5DE]">{companies.map((company) => { const latest = [...company.researchRuns].sort((left, right) => right.startedAt.localeCompare(left.startedAt))[0]; const status = displayStatus(company); return <article className="grid grid-cols-[minmax(0,1fr)_160px_170px_120px] items-center gap-5 px-6 py-5 transition hover:bg-[#FFF9F4]" key={company.id}><div className="min-w-0"><h2 className="truncate font-semibold text-[#25292E]">{company.name}</h2><p className="mt-1 truncate text-sm text-[#7B756D]">{company.domain}</p></div><div><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${statusClasses[status]}`}>{status}</span></div><time className="text-sm text-[#6F6A64]">{latest ? formatTime(latest.finishedAt) : 'Not started'}</time><div className="text-right"><button className="rounded-lg border border-[#D65A1B] bg-white px-3.5 py-2 text-xs font-semibold text-[#A64212] transition hover:bg-[#FFF1E8] focus:outline-none focus:ring-2 focus:ring-[#E86722]/30" onClick={() => onOpen(company)}>Open research</button></div></article> })}</div>
  </section>
}
