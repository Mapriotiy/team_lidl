...
function CompanyResearch({ company, onBack }: { company: CompanyDetail; onBack: () => void }) {
  ...
  return <div className="text-[#20242A]"><button className="text-sm font-semibold text-[#A64212] hover:text-[#742D0D]" onClick={onBack}>← Back to company research</button><div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_18rem]"><main className="min-w-0">
    ...
    <CollectedSources company={company} /></main><aside className="xl:pt-6"><div className="sticky top-6 rounded-2xl border border-[#D8D1C7] bg-[#F7F3ED] p-5"><p className="text-xs font-bold uppercase tracking-wider text-[#A44818]">Research notes</p><h2 className="mt-2 font-semibold">Collection and assessment</h2><p className="mt-3 text-sm leading-6 text-[#68645F]">{company.researchRuns[0]?.warning || 'Collection completed without reported warnings.'}</p></div></aside></div>
  </div>
}
