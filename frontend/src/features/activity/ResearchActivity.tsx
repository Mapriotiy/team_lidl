...
export function ResearchActivity({ runs, showWarnings = true }: { runs: ResearchRun[]; showWarnings?: boolean }) {
  return (
      ...
      <div className="mt-5 space-y-5">
        {runs.map((run) => (
          <article className="relative border-l border-slate-700 pl-5" key={run.id}>
            ...
            {showWarnings && run.warning && <p className="mt-2 rounded-lg bg-amber-300/5 px-3 py-2 text-xs leading-5 text-amber-100/70">{run.warning}</p>}
          </article>
        ))}
      </div>
    </section>
...
function CompanyResearch({ company, onBack }: { company: CompanyDetail; onBack: () => void }) {
  ...
  const supportedSignals = company.assessments.filter((assessment) => assessment.status === 'supported').length
  return <div className="text-[#20242A]"><button className="text-sm font-semibold text-[#A64212] hover:text-[#742D0D]" onClick={onBack}>← Back to company research</button>
    ...
    <section aria-label="Research summary" className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[[`${company.sources?.length ?? 0}`, 'Public sources'], [`${company.evidence.length}`, 'Verified facts'], [`${supportedSignals}/${company.assessments.length}`, 'Supported signals'], [`${Math.round(company.coverage * 100)}%`, 'Signal coverage']].map(([value, label]) => <article className="rounded-xl border border-[#DED9D1] bg-white p-4" key={label}><p className="text-2xl font-semibold text-[#30343A]">{value}</p><p className="mt-1 text-xs font-semibold uppercase tracking-wider text-[#716C65]">{label}</p></article>)}</section>
    <section className="mt-8"><h2 className="text-lg font-semibold">Signals and supporting facts</h2><p className="mt-1 text-sm text-[#716C65]">Each conclusion stays attached to the exact public facts behind it. Signals guide account validation; they do not claim confirmed buying intent.</p>{company.assessments.length ? <div className="mt-5 space-y-5">{company.assessments.map((assessment) => <SignalFactCard assessment={assessment} evidence={assessment.evidenceIds.flatMap((id) => { const item = evidenceById.get(id); return item ? [item] : [] })} key={assessment.id} />)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-[#CEC7BD] bg-white p-10 text-center text-[#68645F]">No signals have been assessed for this company yet.</div>}</section>
    <section className="mt-8"><h2 className="text-lg font-semibold">Research history and diagnostics</h2><p className="mt-1 text-sm text-[#716C65]">Collection coverage, assessment progress, and provider details for each run. Current warnings are shown above.</p><div className="mt-4"><ResearchActivity runs={company.researchRuns} showWarnings={false} /></div></section>
