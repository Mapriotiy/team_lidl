...
function Stage({ run, name }: { run: ResearchRun; name: string }) {
  ...
  const unit = name === 'collection' ? 'pages' : 'signals'
  return <div className="rounded-xl bg-slate-900 p-3"><p className="text-xs font-semibold capitalize text-slate-300">{name}</p><p className="mt-1 text-sm text-white">{progress ? `${progress.completed}${progress.total > 0 ? ` / ${progress.total}` : ''} ${unit}` : 'Waiting'}</p></div>
}

export function ResearchLauncher({ companies, onOpenCompany }: { companies: ConfirmedCompany[]; onOpenCompany?: (companyId: string) => void }) {
