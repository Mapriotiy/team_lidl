...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between gap-2 rounded-xl border border-input bg-card px-3 py-2",
      "text-sm text-foreground data-[placeholder]:text-muted-foreground",
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      position={position}
      className={cn(
        "relative z-50 max-h-80 min-w-[10rem] overflow-hidden rounded-xl border border-border bg-popover",
        "text-popover-foreground shadow-[0px_10px_30px_-10px_rgba(0,0,0,0.25)]",
